#!/bin/bash
cd ~/Escritorio/PCD/aclImdb/train/neg
for file in .txt; do  
...
awk '{getline url < urls_neg.txt; print url, $2}' ../sort_id.txt > ../urls_scores.txt

awk '{scores[$1] += $2; count[$1]++}
    END {for (url in scores) print url, scores[url]count[url]}' 
    ../urls_scores.txt > ../avg_urls_scores.txt

sed 's/\/usercomments//g' ..avg_urls_scores.txt | sort -k2,2n | head -n 10 > ..10peores.txt
